...
if __name__ == "__main__":
    ...
    # ===============================
    # Split train/val para validação justa (estratificada)
    # ===============================
    Xtr_full, Xval, ytr_full, yval = train_test_split(
        X_all, y_all, test_size=VAL_SIZE, random_state=SEED, stratify=y_all
    )
    print("[INFO] Train (full):", Xtr_full.shape, "Val:", Xval.shape)
    print("[INFO] Validação - distribuição de classes:", np.bincount(yval))

    # ===============================
    # Downsampling da classe 0 (apenas no treino!)
    # ===============================
    mask0 = (ytr_full == 0)
    mask1 = (ytr_full == 1)
    n1 = mask1.sum()
    ...
    idx0_down = np.random.choice(idx0, size=n1, replace=False)
    idx_balanced = np.concatenate([idx0_down, idx1])
    np.random.shuffle(idx_balanced)
    Xtr = Xtr_full.iloc[idx_balanced].reset_index(drop=True)
    ytr = ytr_full[idx_balanced]
    print("[INFO] Após downsampling: Xtr:", Xtr.shape, "ytr counts:", np.bincount(ytr))
    ...
    rf = RandomForestClassifier(
        ...
        max_depth=12,
        n_jobs=-1,
        class_weight=None,  # já balanceamos manualmente!
        random_state=SEED
    )
    t0 = time.time()
    rf.fit(Xtr_s, ytr)
    ...
    xgb_model = xgb.XGBClassifier(
        ...
        scale_pos_weight=1.0,  # balanceado manualmente!
        tree_method="hist",
        random_state=SEED,
        n_jobs=-1,
        verbosity=1,
    ...
    # ===============================
    # Treinando com todos os dados balanceados (opcional)
    # ===============================
    # Se quiser, re-treine com X_all (downsampled) + Xval juntos, antes de produzir a submissão.
