...
NU = 0.0185  # Aumentado para evitar colapso
WARMUP_EPOCHS = 1
EPOCHS = 0  # Aumentado para permitir refino
LR = 1e-3
...
def train_model(net, dl_norm, c, R2, epochs=1, lr=1e-3, wd=1e-5, device="cuda"):
    ...
    for ep in range(1, epochs+1):
        ...
        for (xb,) in dl_norm:
            ...
            # Loss do Deep-SVDD para Fixed-R. Incentiva a dispersão.
            loss = torch.mean(torch.clamp(dist_sq, min=R2))
...
def calibrate_threshold_by_validation(scores_val, yval, quantiles=None):
    ...
    best_f1_macro, best_thr = -1.0, None
    for q in quantiles:
        thr = np.quantile(base, q)
        yhat = (scores_val > thr).astype(int)
        f1m = f1_score(yval, yhat, average="macro")
        if f1m > best_f1_macro:
            best_f1_macro, best_thr = f1m, float(thr)

    print(f"[INFO] Calibração validação: melhor F1-macro={best_f1_macro:.6f} @ thr={best_thr:.6e}")
    return best_thr, best_f1_macro
...
if __name__ == "__main__":
    ...
    train_model(net, dl_norm, c, R2=0, epochs=WARMUP_EPOCHS, lr=LR, wd=WD, device=device) # R2=0 para o warmup
    ...
    # (Opcional: refino)
    if EPOCHS > 0:
        print(f"[INFO] Refinando embedding (perda SVDD) por {EPOCHS} épocas ...")
        train_model(net, dl_norm, c, R2, epochs=EPOCHS, lr=LR, wd=WD, device=device)
    ...
    train_model(net_final, dl_norm_full, c_final, R2=0, epochs=WARMUP_EPOCHS, lr=LR, wd=WD, device=device)
    ...
    if EPOCHS > 0:
        ...
        train_model(net_final, dl_norm_full, c_final, R2_final, epochs=EPOCHS, lr=LR, wd=WD, device=device)
