...
# + NORMALIZAÇÃO POR GRUPO (tr_src, tr_dst) dos scores (z-score por grupo com estatísticas da VAL)
# + OTIMIZAÇÃO DO F1-SCORE DA CLASSE MINORITÁRIA NA CALIBRAÇÃO
...
CACHE_TAG   = "DELTA1"  # usar DELTA1 (sua melhor base)
USE_CACHE   = True
CSV_ENGINE  = "pyarrow"
...
# Mahalanobis
SHRINKAGE = 0.05  # bom default de antes

# Normalização por grupo
MIN_GROUP_NORMALS = 100     # mínimo de normais na VAL para estimar média/std do grupo
EPS_STD             = 1e-8    # proteção para std muito pequeno
...
class DeepSVDDNet(nn.Module):
    def __init__(self, d_in):
        super().__init__()
        # Aumentei a capacidade da rede para potencialmente capturar padrões mais complexos
...
def calibrate_threshold_by_validation(scores_val, yval):
    """
    Função modificada para otimizar diretamente o F1-score da classe minoritária (y=1).
    Isso deve aumentar o recall e o f1-score da classe 1.
    """
    ...
    # Precisão e recall da classe 1 (positiva)
...
if __name__ == "__main__":
    ...
    # ----- Leitura CSVs (mantemos os DataFrames para extrair chaves de rota)
    t0 = time.time()
    ...
    # ----- Chaves de rota (inteiros estáveis) para TREINO e TESTE
    rk_all  = make_route_keys(train_df)  # [N_train]
    rk_test = make_route_keys(test_df)   # [N_test]
    ...
    # ATENÇÃO: A função foi modificada para otimizar o F1-score da classe 1.
    ...
    dl_test_onlyX = dl_test  # já é somente X
