# svdd_mlab_oneclass_gpu_calibrated_deltas_v2.py
# Deep-SVDD one-class (FIXED-R + calibração exata) com deltas enriquecidos
...
CACHE_TAG   = "DELTA2"   # novo cache para features novas
USE_CACHE   = True
CSV_ENGINE  = "pyarrow"
...
# =======================
# Featurização com DELTAS enriquecidos
# =======================
...
def featurize_fast(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # --- DELTAS por (tr_src, tr_dst)
    required = {"tr_src","tr_dst","seconds_since_start"}
    if required.issubset(df.columns):
        base_cols = ["rtt_mean","rtt_p90","success_rate","replies_per_attempt",
                     "rtt_std","rtt_median"]   # << NOVO: std e median
        ...
        # time_since_prev (antes para usar nos per-second)
        ...
        def add_delta_and_ratio(col, clip_ratio=(0.0, 10.0)):
            ...
            # delta por segundo (evita explosões quando gap é grande)
...
if __name__ == "__main__":
    ...
    # ----- Calibração EXATA com o MODELO FINAL -----
    print("[INFO] Recalibrando limiar na validação (EXATO) com o MODELO FINAL ...")
