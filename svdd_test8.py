# svdd_mlab_oneclass_gpu_calibrated_deltas.py
# Deep-SVDD one-class (FIXED-R + calibração exata) para mudança de rota (M-Lab)
# - Featurização com DELTAS por (tr_src, tr_dst)
# - NU opcionalmente automático (proporção de 1s no treino)
# - Warm-up curto, sem refino (EPOCHS=0)
...
CACHE_TAG   = "DELTA1"   # evita reaproveitar cache antigo sem deltas
USE_CACHE   = True
CSV_ENGINE  = "pyarrow"
...
def featurize_fast(df: pd.DataFrame) -> pd.DataFrame:
    t0 = time.time()
    # --- estatísticas absolutas (como antes)
    ...
    # --- DELTAS por (tr_src, tr_dst) com ordenação temporal
